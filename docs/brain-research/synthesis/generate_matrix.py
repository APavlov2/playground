...
def build():
    ...
    for fm in entries:
        ...
        agents.append(dict(
            ...
            klass=fm.get("klass", ""), ev=fm["evidence_overall"], ch=ch_map,
        ))
        for c in chans:
...
def write_briefs(bychan, meta):
    ...
    for cid in CH_IDS:
        ...
        L.append("| Agent | Type | Overall | Channel ev | Dir | Population | Mechanism |")
        L.append("|-------|------|:-------:|:----------:|:---:|------------|-----------|")
        for r in rows:
            mech = r["mechanism"].replace("|", "\\|")
            if len(mech) > 130:
                mech = mech[:127] + "…"
            L.append(f"| [{r['name']}](../{r['type']}s/{r['id']}.md) | {r['type']} | {r['overall']} "
